#############################################################################
# Model weight generator for the first level models in the validation logic #
#############################################################################
...
monthsBackModels <- 1:15
nbMonths <- length(monthsBackModels)
# monthsBackModelsWeights <- rev(c(12, 0.1*(5:15)))
monthsBackWeightDates <- rev(as.Date(paste(c(rep(2015, 11), rep(2016, 4)),
                                           str_pad(c(2:12, 1:4), 2, pad='0'),
                                           28, sep="-")))
# Target weight sum and default weights
monthsBackModelsWeights <- rev(c(1e-10, 1e-10, 0.1*(12:24)))
...
saveRDS(modelWeights, file.path(saveFolder,
