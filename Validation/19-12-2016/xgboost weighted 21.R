############################################
# XGBOOST weighted 21 - Validation on 2016 #
############################################
...
loadFile <- "" # "xgboost weighted trainAll 21, validation Mar16" # "xgboost weighted trainAll 17, validation May16"
submissionFile <- paste0("xgboost weighted trainAll 21, validation ",
                         studiedMonth, studiedYear)
...
validationFeaturesFolder <- paste("validation Back", monthsBackAnalysis)
...
# Validation - exclude where back >= monthsBackAnalysis
monthsBackGroups <- as.numeric(gsub("^.*Back|Lag.*$", "", modelGroups))
modelGroups <- modelGroups[monthsBackGroups>monthsBackAnalysis]
  ...
  if(loadPredictions && file.exists(rawPredictionsPath)){
    ...
    for(lagId in 1:nbConditionalLags){
      ...
      for(i in 1:nbBaseModels){
        ...
        # Show progress message
        cat("Generating validation predictions for model", i, "of",
            nbBaseModels, "\n")
    ...
    if(saveAllPredictionsPosFlanks && length(allPredictionsPosFlanks)>0 &&
       length(allPredictionsPosFlanks[[1]])>0){
