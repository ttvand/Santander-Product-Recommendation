shinyServer(function(input, output, session) {
  ...
  # Logic to subset the base and self predictions of the selected product
  combinedPredictionsProduct <- reactive({
    inspectProduct <- paste0("ind_", input$targetProduct, "_ult1")
    baseModelPredictionsTarget <-
    ...
    baseModelPredictionsTarget <- baseModelPredictionsTarget[positiveRows]
    selfPredictions <- selfPredictions[positiveRows]
    
    # Combine the base predictions with the self predictions
    out <- cbind(baseModelPredictionsTarget[
      , ((ncol(baseModelPredictionsTarget)-2):1), with=FALSE], selfPredictions)
    
    out
  })
  
  # Return the true positive flank status for the selected product
  posFlankInspected <- reactive({
    inspectProduct <- paste0("ind_", input$targetProduct, "_ult1")
    selfPosFlanks <- allPredsSelf[product == inspectProduct, posFlank]
    out <- selfPosFlanks[input$inspectRow]
    out
  })
  
  # Return the weights of the base month predictions for the selected product
  lagWeightsProduct <- reactive({
    inspectProduct <- paste0("ind_", input$targetProduct, "_ult1")
    dateTargetWeightsProd <- dateTargetWeights[targetProduct == inspectProduct]
    out <- dateTargetWeightsProd[modelLag>2, relativeWeight]
    out <- out/sum(out)
    out
  })
  ...
  # Previous inspect row logic
  observeEvent(input$prevRow,{
    currentId <- input$inspectRow
    newId <- max(c(1, currentId - 1))
    
    # Update target product
    updateSelectInput(session, "inspectRow",
                      selected = newId)
  })
  
  # Next inspect row logic
  observeEvent(input$nextRow,{
    currentId <- input$inspectRow
    newId <- min(c(nrow(combinedPredictionsProduct()), currentId + 1))
  ...
  # Plot the lag probabilities with the self probability for the selected
  # product and data row
  output$countByLagPlotly <- renderPlotly({
    plotData <- combinedPredictionsProduct()
    if(nrow(plotData) == 0) return()
    plotRow <- as.matrix(plotData[input$inspectRow])[1, ]
    lags <- as.numeric(gsub(".*Lag", "", names(plotRow)[-length(plotRow)]))
    plotData <- data.frame(Lag = c(lags, max(lags)+1),
                           Probability = plotRow,
                           Connection = factor(c(rep("Historical", length(lags)),
                                                 "Self"),
                                               levels = c("Historical", "Self",
                                                          "Weighted",
                                                          "Adjusted")))
    plotData <- rbind(plotData, plotData[nrow(plotData), ])
    plotData$Connection[nrow(plotData)] <- "Weighted"
    plotData <- rbind(plotData, plotData[nrow(plotData), ])
    plotData$Connection[nrow(plotData)] <- "Adjusted"
    plotDataSelf <- data.frame(Lag = c(max(lags), 1 + max(lags)),
                               Probability = plotRow[-(1:(length(plotRow)-2))],
                               Connection = rep("Self", 2))
    
    # Extract the actual positive flank status
    posFlankInspected <- posFlankInspected()
    
    p <- ggplot(plotData, aes(x=Lag, y=Probability, colour = Connection)) +
      geom_line() +
      theme(axis.text.x=element_text(angle = -90, hjust = 0, vjust = 0.5)) +
      guides(fill=FALSE) +
      ylim(c(0, max(plotData$Probability))) + 
      ggtitle(paste("Positive flank status:", posFlankInspected))
    
    # Add the weighted prediction
    lagWeightsProduct <- lagWeightsProduct()
    weightedPrediction <- sum(lagWeightsProduct * plotRow[-length(plotRow)])
    plotDataWeighted <- data.frame(Lag = c(max(lags), 1 + max(lags)),
                                   Probability = c(plotRow[length(plotRow)-1],
                                                   weightedPrediction),
                                   Connection = rep("Weighted", 2))
    p <- p +
      geom_line(data=plotDataWeighted)
    
    # Add the adjusted prediction
    lagProbs <- plotRow[-length(plotRow)]
    meanLastLags <- mean(lagProbs[-(length(plotRow)-(lastLagsWeightSelection+1):
                                      length(plotRow))])
    validLagIds <- lagProbs > meanLastLags/validLagRelCutoff &
      lagProbs < meanLastLags*validLagRelCutoff
    adjustedProb <- sum(lagWeightsProduct[validLagIds] *
                          lagProbs[validLagIds])/
      sum(lagWeightsProduct[validLagIds])
    plotDataAdjusted <- data.frame(Lag = c(max(lags), 1 + max(lags)),
                                   Probability = c(plotRow[length(plotRow)-1],
                                                   adjustedProb),
                                   Connection = rep("Adjusted", 2))
    p <- p +
      geom_line(data=plotDataAdjusted)
    
    # Add the self prediction
    p <- p +
      geom_line(data=plotDataSelf)
    
    # Draw a vertical line to separate the last three historical lags
    p <- p +
