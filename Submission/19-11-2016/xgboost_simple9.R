...
submissionDate <- "19-11-2016"
submissionFile <- "xgboost 9 back 11-11 Zero deco_fin dela_fin normalizeJune16"
...
meanGroupPreds <- c(mean(newProdPredictions[testData$hasMay15Data==0]),
...
for(i in 1:nbBaseModels){
  ...
  if(targetVar %in% zeroTargets){
    ...
    if(normalizeRRJune16){
      targetRelMapCol <- match(targetVar, colnames(mapContributions))
      June16Ratio <- mapContributions[17, targetRelMapCol] / 
        mean(predictionsPrevNotOwned)
      # prevZeroFraction <- (1 - mean(alreadyOwned))
      prevZeroFraction <- sum(newProdPredictions[!alreadyOwned]) /
        sum(newProdPredictions)
      probMultiplier <- June16Ratio/prevZeroFraction
      if(is.finite(probMultiplier)){
        predictions <- predictions * probMultiplier
      }
    }
  }
