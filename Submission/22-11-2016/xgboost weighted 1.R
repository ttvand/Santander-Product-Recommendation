##################################
# XGBOOST weighted predictions 1 #
##################################
...
submissionDate <- "22-11-2016"
loadFile <- "xgboost weighted back 11-0 zero prods 10, 11, 12 and 14 not normalised"
submissionFile <- "xgboost weighted back 11-0 zero prods 10, 11, 12 and 14 linear normalisation no zeroing"
...
normalizeMode <- c("linear", "exponential")[1]
...
normalisedProbRatio <- expectedPosFlanks/leaderboardPosFlanks
cat("Expected/leaderboard positive flank ratio",
    round(normalisedProbRatio, 2), "\n")
...
if(normalizeProdProbs){
  for(i in 1:nbBaseModels){
    ...
    probMultiplier <- nrow(testDataLag) * fractionPosFlank *
      countContributions[17, i] / predictedPosFlankCount * normalisedProbRatio
    probMultipliers[i] <- probMultiplier
    if(i %in% c(3, 5, 7, 13, 18, 19, 22, 23, 24)) browser()
    if(is.finite(probMultiplier)){
      if(targetVar %in% additiveNormalizeProds){
...
allPredictions[, totalProb := prediction * newProdPredictions]
