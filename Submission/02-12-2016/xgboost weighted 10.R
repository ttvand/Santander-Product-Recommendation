###################################
# XGBOOST weighted predictions 10 #
###################################
...
submissionDate <- "02-12-2016"
loadFile <- "xgboost weighted trainAll 10, linear increase jun15 times6 back 13-0 no zeroing, exponential normalisation joint"
submissionFile <- "xgboost weighted trainAll 10, underSampleNomPensNoNomina linear increase jun15 times6 back 13-0 no zeroing, exponential normalisation joint"
...
for(i in 1:nbModelGroups){
  ...
  for(j in 1:nbModels){
    ...
    foldModelWeight <- ifelse(grepl("Fold", modelGroupFile),
                              foldRelativeWeight, 1)
