...
submissionDate <- "18-11-2016"
# submissionFile <- "xgboost simple 7 back 11-11 trainAll Zero deco_fin dela_fin"
# submissionFile <- "xgboost simple 7 back 11-11 trainAll multiplyRRJune16 Exp 1"
submissionFile <- "xgboost simple 7 back 11-11 multiplyRRJune16 Exp 0.6"
...
for(i in 1:nbBaseModels){
  ...
  if(targetVar %in% zeroTargets){
    ...
    if(multiplyRRJune16){
      June16Ratio <- testData[1, paste0(targetVar, "MAPRatioJune15"),
                              with=FALSE][[1]]
      if(is.finite(June16Ratio)){
        predictions <- predictions * (June16Ratio)^June16Exponent
      }
    }
  }
